...
`tools/sync-audio-words.mjs` 將上述兩個工具串成排程用的一鍵流程：
先切換並 `git pull --ff-only` 目標分支（預設 `n8n-test`），依序執行
`update-audio-words` 與 `merge-audio-words`，若 `public/assets/data` 或
`public/assets/audio-words` 有異動，就以「`yyyy/mm/dd` 自動對齊音檔時間軸」
為訊息自動 commit 並推回同一分支；沒有異動則直接結束。

```bash
# 排程用：處理最近兩個月並自動 commit＋push
npm run sync-audio-words

# 本機測試：不拉取、不推送，只跑工具與 commit
npm run sync-audio-words -- --no-pull --no-push
```

為避免把不相關的變更混進自動 commit，工作目錄有其他異動時預設會中止，
可用 `--allow-dirty` 略過檢查（仍只會提交上述兩個資料夾）；任一工具失敗
（exit code 非零）時會跳過 commit 與 push，已完成的部分保留在工作目錄，
排除問題後重跑即可續作。執行 `npm run sync-audio-words:help` 查看
`--branch`、`--remote` 等所有參數。
